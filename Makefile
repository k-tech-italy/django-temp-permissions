...
static:  ## build static assets
	@rm -fr src/django_temporary_permissions/web/static/*
	@sass src/django_temporary_permissions/web/assets/tw.in.scss | postcss  --config   -o src/django_temporary_permissions/web/assets/tw.css
	@node_modules/.bin/webpack --mode ${NODE_ENV} --progress  --bail
	@STATIC_ROOT=src/django_temporary_permissions/web/static ./manage.py collectstatic_js_reverse -v 0
	@STATIC_ROOT=src/django_temporary_permissions/web/static ./manage.py collectstatic --no-input -v 0
	@git add src/django_temporary_permissions/web/static
	@echo $(shell /bin/ls -alu src/django_temporary_permissions/web/static/django_temporary_permissions/app.js)
	@echo $(shell /bin/ls -alu src/django_temporary_permissions/web/static/django_temporary_permissions/app.css)
...
reset-migrations: ## reset django migrations
	...
	rm -f *.db
	./manage.py makemigrations django_temporary_permissions
	./manage.py makemigrations --check
	@echo "\033[31;1m You almost there:"
	@echo "\033[37;1m  - add 'CITextExtension()' as first operations of src/django_temporary_permissions/migrations/0001_initial.py"
