...
from django_temporary_permissions.admin import TemporaryPermissionAdmin
...
def test_check_overlaps_detected_button(admin_client, monkeypatch):
    monkeypatch.setattr("django_temporary_permissions.signals.permission_overlap_detected.send", mock := Mock())
    ...
    change_url = reverse("admin:django_temporary_permissions_temporarypermission_change", args=[temp_perm_1.id])
    admin_client.get(change_url)

    # click button in the admin
    response = admin_client.get(
        reverse("admin:django_temporary_permissions_temporarypermission_check_overlaps", args=[temp_perm_1.id]),
        follow=True,
        HTTP_REFERER=change_url,  # django-admin-extra buttons redirects implicitly to HTTP_REFERRER
    )

    link_to_admin = reverse("admin:django_temporary_permissions_temporarypermission_change", args=[temp_perm_2.id])
...
def test_check_overlaps_add_view(admin_client, monkeypatch):
    ...
    response = admin_client.post(
        reverse("admin:django_temporary_permissions_temporarypermission_add"),
        data=data,
        follow=True,
    )
...
def test_check_overlaps_change_view(admin_client, monkeypatch):
    ...
    response = admin_client.post(
        reverse("admin:django_temporary_permissions_temporarypermission_change", kwargs={"object_id": temp_perm_2.id}),
        data=data,
        follow=True,
    )
...
def test_configure_temporary_permission_button(admin_client):
    ...
    assert redirect_url == f"{reverse('admin:django_temporary_permissions_temporarypermission_add')}?user={user.id}"
